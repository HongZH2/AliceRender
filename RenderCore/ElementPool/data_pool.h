...
public:
    ...
    static std::shared_ptr<DataBlock<T>> getBlock(){  // apply for a buffer block 申请一块float block
    ...
    template<typename T>
    static std::shared_ptr<DataBlock<T>> getSingleBlock(){ // get an extra and unorganized block
        std::shared_ptr<DataBlock<T>> block = std::make_shared<DataBlock<T>>();
        return block; 
    }  
